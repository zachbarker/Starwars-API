...
const VehicleScreen = () => {
    ...
    const onloadHelper = async () => {
        const response = await fetch(baseURL + 'vehicles/?search=') 
        const data = await response.json() 
        const res = data.results 
        setCurrentItem(res) 
    }
    // calls onloadhelper on load
    useEffect(() => {
        onloadHelper()
    }, [])
    ...
    const handleSearch = async() => {
        ...
        const response = await fetch(baseURL + 'vehicles/?search=' + input) 
                ...
                {currentItem.map((currentItem, index) => (
                    <div key={index} className="card">
                        <div className="card-body">
                            ...
                            {currentItem.cost_in_credits == 'unknown' && (<p className="card-text">Cost: {currentItem.cost_in_credits}</p>)}
                            {currentItem.cost_in_credits != 'unknown' && (<p className="card-text">Cost: {currentItem.cost_in_credits} credits</p>)}
                            <p className="card-text">Manufacturer: {currentItem.manufacturer}</p>
                            <p className="card-text">Top Speed: {currentItem.max_atmosphering_speed} km/h</p>
                            <p className="card-text">Class: {currentItem.vehicle_class}</p>
                            <p className="card-text">Length: {currentItem.length}m</p>
